@extends('layouts.default')
@section('title')
About
@endsection
@section('body')
	<div class="class-creation-form color-a">
	    <div class="descr_title"><h3>Arc Notes</h3></div>
	    <div class="descr_content">Write and share all your personal course notes with your classmates.</div>
	    <div class="descr_content">Access to all others classmates personal notes.</div>
	    <br/>
        <div class="origin_title"><h3>Origin</h3></div>
        <div class="origin_content">Originally create in the context of school projects in third year at He-Arc.</div>
        <br/>
        <div class="creator_title"><h3>Creators</h3></div>
        <div class="creator_content">
            Kilian le greudin
            Nicolas huge nains
            Vincent Des Rouages
        </div>
	</div>
</body>
</html>
@stop